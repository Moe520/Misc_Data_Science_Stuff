...
def get_tweet_list(twapi, idlist):
    ...
    for tweet in tweets:
        ...
        with open('output.txt', 'a') as outfile:
            outfile.write('%s,"%s",%s ,"%s",%s,%s,%s,%s,"%s","%s",%s ' % (tweet.id , tweet.text.encode('UTF-8'),tweet.created_at,tweet.user.screen_name.encode('UTF-8'),tweet.user.followers_count,tweet.user.friends_count,tweet.user.statuses_count,tweet.user.favourites_count,tweet.user.location.encode('UTF-8'),tweet.user.name.encode('UTF-8'),'\n'))
